...
def run(devices, show_commands=None, cfg_commands=None):
    results = []
    if show_commands:
        results += [send_show_commands_to_cisco(device, show_commands)
                    for device in devices]
    if cfg_commands:
        results += [send_cfg_commands_to_cisco(device, cfg_commands)
                    for device in devices]
    return results
